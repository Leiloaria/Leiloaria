...
public class LeilaoService implements LeilaoServiceI {
    ...
    public void atualizarStatusLeilao(Long leilaoId, StatusLeilao novoStatus) {
        ...
        if (leilao.getStatus() == StatusLeilao.PENDENTE
                && (novoStatus == StatusLeilao.FINALIZADO || novoStatus == StatusLeilao.AGUARDANDO_PAGAMENTO)) {
            throw new AtualizarLeilaoInvalidoException(
                    "Leilão está pendente e deve ser aberto antes de ser finalizado");
        }
    ...
    public Page<Leilao> listarLeiloesPorParticipanteId(Long participanteId, Pageable pageable) {
        usuarioRepo.findById(participanteId)
                .orElseThrow(() -> new RecursoNaoEncontradoException("Usuario não encontrado"));

        return repository.findByLoteLancesUsuarioId(participanteId, pageable);
    }
}
